...
function getSupabase() {
  if (!supabase) {
    throw new Error('SUPABASE_SERVICE_KEY or SUPABASE_KEY is required');
  }
  return supabase;
}

function markListingsSeen(seenMap, listings, timestamp) {
  var seenAt = timestamp || Date.now();
  listings.forEach(l => { seenMap[l.address] = seenAt; });
  return seenMap;
}

function persistSeenAddresses(seenMap, listings) {
  markListingsSeen(seenMap, listings);
...
async function storeReport(report, db) {
  var client = db || getSupabase();
  ...
  for (var j = 0; j < report.properties.length; j++) {
    ...
    var { error } = await client.from('property_searches')
  ...
  var { error: reportError } = await client.from('property_reports').insert({
...
async function run() {
  ...
  // 10. Store in Supabase. If storage fails, abort before saving the
  // dedup map so today's listings are retried on the next run.
  await storeReport(report);
  
  // Persist the seen hashmap only after Supabase accepted the report.
  persistSeenAddresses(seenMap, allListings);
